...
{
    public class HtmlParagraph_Tests
    {
        #region ContainsHtmlLineBreak
        //public static readonly Regex HTML_LINEBREAK = new Regex(@"<\s*/?\s*br\s*/?>", RegexOptions.Compiled);
        public static readonly Regex HTML_Paragraph = new Regex(@"<\s*/?\s*br\s*/?><\s*/?\s*br\s*/?>", RegexOptions.Compiled);
        private static bool ContainsHtmlLineBreak(string text) => HTML_Paragraph.Matches(text).Any();
